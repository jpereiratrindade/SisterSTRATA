...
bool TerrainGenerator::generate(const std::string& filename, int width, int height, float spacing, Type type, std::function<void(float, const std::string&)> onProgress) {
    ...
    if (ext == ".csv" || ext == ".txt" || ext == ".xyz") {
        // Write Point Cloud (Grid preserves structure)
        file << "x,y,z,r,g,b\n"; // Header
        for (const auto& v : vertices) {
            // Default color white/gray
            file << v.x << "," << v.y << "," << v.z << ",0.5,0.5,0.5\n"; // Simple CSV
        }
        std::cout << "[Generator] CSV Written. " << vertices.size() << " points." << std::endl;
    } else {
        // Write OBJ (Default)
        ...
        for (const auto& f : faces) {
         ...
         std::cout << "[Generator] OBJ Written. " << vertices.size() << " vertices, " << faces.size() << " faces." << std::endl;
    }
